			...
			try {
				if (typeof usi_js !== 'undefined' && typeof usi_js.cleanup === 'function') {
					...
					if (usi_app.is_image() || usi_app.is_image_in_cart) {
						...
						if ((usi_cookies.value_exists("usi_subtotal") && Number(usi_cookies.get("usi_subtotal")) <= 49) && usi_app.can_flex && usi_date.is_after('2022-12-01T06:29')) {
							//Flex Trial
							usi_commons.log('[ load ] * * * Load Images Flex Trial [language key: ' + language_key + '] * * *');
							usi_commons.load_view("K32I17tav4v8GDZH05GEZ2J", "43971", usi_commons.device + language_key);
						} else {
							// Images 10% Off
						...
						//********* Footage / Video Area
						var language_key = usi_app.get_language_key();
						if (usi_app.get_messaging("footage_tenpercent")) {
							if (usi_cookies.value_exists('usi_prod_pid_1') && usi_cookies.value_exists('usi_prod_name_1') && usi_cookies.value_exists('usi_prod_image_1')) {
								if((usi_cookies.value_exists("usi_subtotal") && Number(usi_cookies.get("usi_subtotal")) <= 49) && usi_app.can_flex && usi_date.is_after('2022-12-01T06:29')){
									//Flex Trial
									usi_commons.log('[ load ] * * * Load Video Flex Trial [language key: ' + language_key + '] * * *');
									usi_commons.load_view("Uw4YC4xp7Q6RP6gORu9ZBA0", "43915", usi_commons.device + language_key);
								}else{
									...
									if (split_key == "dynamic") {
										...
										usi_commons.log('[ load ] * * * Load 10% Off Creative Refresh (Cart Only) TT [language: ' + language_key + '] * * *');
										usi_app.messaging = usi_app.get_messaging("footage_tenpercent");
										if (Math.random() > .5) {
											//Static
											usi_commons.load_view("CScPbydcZsSjicirlHmdYjH", "44267", usi_commons.device + language_key);
										} else {
											//Video
											usi_commons.load_view("8GS6i5Gbsdh5coOV0WTpVxc", "44263", usi_commons.device + language_key);
										}
									}
								}
							} else {
								usi_commons.log('[ load ] * * * Load 10% Off Creative Refresh TT [language: ' + language_key + '] * * *');
								usi_app.messaging = usi_app.get_messaging("footage_tenpercent");
								if (Math.random() > .5) {
									//Static
						...
						if ((usi_cookies.value_exists("usi_subtotal") && Number(usi_cookies.get("usi_subtotal")) <= 49) && usi_app.can_flex && usi_date.is_after('2022-12-01T06:29')) {
							//Flex Trial
							usi_commons.log('[ load ] * * * Load Music Flex Trial [language key: ' + language_key + '] * * *');
							usi_commons.load_view("I1YZtIjZFsgqRDX6FQ6MjeK", "43969", usi_commons.device + language_key);
